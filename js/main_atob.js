...
function filter(pageTitle,year,exp_start,exp_end,selected_exp_loop){
	...
	$.each($("#city_b option:selected"), function(){
                selected_city_b.push($(this).val());
        });
...
if( selected_city_a.length > 0 && typeof selected_city_a != "undefined"){
                console.log(selected_city_a)
        log['Selected_City_A'] = selected_city_a;

}

if( selected_city_b.length > 0 && typeof selected_city_b != "undefined"){
                console.log(selected_city_b)
        log['Selected_City_B'] = selected_city_b;
...
//get data from the server and filter it according to the filters provided by users  
...
if(selected_city_b != ''){
	//	alert('no city b is selected');

//let res = data.filter(it => it..includes('oli'));

 data_status = data.filter(el => ( 	
				  (
			
							
				selected_city_a.indexOf(el.A_end_site_City) >=0 && 
                                selected_city_b.indexOf(el.B_end_site_City) >= 0
				  ) || 
				(
				selected_city_a.indexOf(el.B_end_site_City) >= 0 &&
                                selected_city_b.indexOf(el.A_end_site_City) >= 0	
				)
